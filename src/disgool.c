...
#define gRefI(i) GOOL_DIS_FormatRef(ostr,i,0)
#define gRefO(i) GOOL_DIS_FormatRef(ostr,i,1)
#define gRefX(i) GOOL_DIS_FormatRef(ostr,i,2)
#define gIm(i)   sprintf(ostr, "%s 0x%x,", ostr, i)
#define gReg(i)  sprintf(ostr, "%s self[0x%x],", ostr, 0x60+(i*4));
#define gLnk(i)  GOOL_DIS_FormatLnk(ostr,i)
#define gVec(i)  GOOL_DIS_FormatVec(ostr,i)
#define gCol(i)  GOOL_DIS_FormatCol(ostr,i)
...
MakeDisFg(GOOL_DIS_Disassemble , disGOOL[ins >> 24](ins, pc);)

char *GOOL_DIS_FormatRef(char *str, uint32_t ref, int mode)
{
  if ((ref & 0xFFF) == 0xE1F)
  {
    if (mode == 0)
...
char *GOOL_DIS_FormatLnk(char *str, uint32_t lnk)
{
...
char *GOOL_DIS_FormatVec(char *str, uint32_t vec)
{
...
char *GOOL_DIS_FormatCol(char *str, uint32_t col)
{
